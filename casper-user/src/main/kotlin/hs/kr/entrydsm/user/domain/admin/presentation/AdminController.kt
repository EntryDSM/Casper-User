package hs.kr.entrydsm.user.domain.admin.presentation

import hs.kr.entrydsm.user.domain.admin.presentation.dto.request.AdminLoginRequest
import hs.kr.entrydsm.user.domain.admin.service.AdminLoginService
import hs.kr.entrydsm.user.domain.admin.service.AdminTokenRefreshService
import hs.kr.entrydsm.user.domain.admin.service.DeleteAllTableService
import hs.kr.entrydsm.user.domain.admin.service.QueryAdminByUUIDService
...
class AdminController(
    ...
    ): TokenResponse = adminLoginService.execute(adminLoginRequest)

    @PutMapping("/auth")
    fun tokenRefresh(
        @RequestHeader("X-Refresh-Token") refreshToken: String,
    ): TokenResponse = adminTokenRefreshService.execute(refreshToken)
